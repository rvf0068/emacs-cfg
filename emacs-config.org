...
(use-package emacs
  ...
  ;; reading buffer and file names are case insensitive
...
This custom minor mode allows for quick insertion of standard math sets ($\mathbb{R}, \mathbb{Q}, \mathbb{C}$, etc.) by simply pressing the corresponding capital letter while inside a math environment.

#+begin_src emacs-lisp
(require 'math-shortcuts)
(add-hook 'LaTeX-mode-hook #'math-shortcuts-mode)
(add-hook 'org-mode-hook #'math-shortcuts-mode)
#+end_src
...
(use-package org
  ...
  :config
  (add-to-list 'org-src-lang-modes '("python-cell" . python))
  ...
  ;; LaTeX/PDF use biblatex's default style (via latexmk+biber); other
  ;; backends (HTML, Markdown, ...) keep org-cite's built-in "basic" style.
...
#+begin_src emacs-lisp
(defvar my-latex-preview-scale 1
  "Scale factor for LaTeX previews. Can be overridden in personal.org.")
...
(use-package org-fragtog
  ...
  (org-preview-latex-image-directory "~/Documents/org-latex-previews/")
  (org-format-latex-header (concat org-format-latex-header "\n\\usepackage{lxfonts}\n\\usepackage{amsmath}\n\\usepackage{amsfonts}"))
  (org-format-latex-options (plist-put org-format-latex-options :scale my-latex-preview-scale))
  :config
...
(use-package eglot
  ...
  ;; Tell default linters to allow 88 characters
